...
def open_form_dialog(url):
    st.write("Submit the form below to update the record.")
...
# RECOVER ADVISOR NAME & MANAGER IF MISSING
if 'Advisor Name' not in dsat_raw.columns:
    dsat_raw = dsat_raw.merge(team_db[['Email', 'Advisor Name', 'Manager Name']], on='Email', how='left', suffixes=('', '_team'))
...
# 1. Sr. Manager / Admin Selection Logic
if level == "Admin":
    st.sidebar.subheader("Organization View")
    sr_mgr = st.sidebar.selectbox("Select Sr. Manager Organization", ["All Organization", "Jarvis Sokolowich", "Sumit Ludhwani"])
    
    if sr_mgr != "All Organization":
        # Filter team_db to only include managers reporting to the selected Sr. Manager
        managers_under_sr = team_db[team_db['Manager Name'] == sr_mgr]['Advisor Name'].unique()
        # Further refine view by specific manager if needed
        m_view = st.sidebar.selectbox(f"Filter {sr_mgr}'s Managers", ["All Teams"] + list(managers_under_sr))
        
        if m_view == "All Teams":
            team_emails = team_db[team_db['Manager Name'].isin(managers_under_sr)]['Email'].unique()
            f_kpi, f_dsat = f_kpi_t[f_kpi_t['Email'].isin(team_emails)], f_dsat_t[f_dsat_t['Email'].isin(team_emails)]
        else:
            team_emails = team_db[team_db['Manager Name'] == m_view]['Email'].unique()
            f_kpi, f_dsat = f_kpi_t[f_kpi_t['Email'].isin(team_emails)], f_dsat_t[f_dsat_t['Email'].isin(team_emails)]
    else:
...
with tabs[0]:
    # Metrics Logic (Rounded to 2 decimals)
    ...
    m = st.columns(5)
    m[0].metric("Avg Shift Score", f"{avg_score:.2f}%"); m[2].metric("Avg Sent Rate %", f"{avg_sent:.2f}%"); m[3].metric("Avg Satisfied Survey", f"{avg_sat:.2f}%")
    m[4].metric("Total Survey", int(f_kpi['Total Survey'].sum()))

    # Trends
    chart_d = f_kpi.groupby('Date_Parsed').mean(numeric_only=True).reset_index()
    if not chart_d.empty: st.plotly_chart(px.line(chart_d, x='Date_Parsed', y='Shift_Score', title="Org Performance Trend", markers=True), use_container_width=True)

with tabs[1]:
    st.markdown("### 🚫 DSAT Analysis & Audit")
    # 2. DSAT Summary including Pending Feedback
    pending_count = len(f_dsat[f_dsat['Feedback'].isna() | (f_dsat['Feedback'].astype(str).str.strip() == "")])
    s1, s2, s3, s4 = st.columns(4)
    s1.metric("Total DSATs", len(f_dsat))
    s2.metric("Feedback Pending", pending_count, delta_color="inverse", delta=f"{pending_count} Unactioned")
    s3.metric("Controllable", len(f_dsat[f_dsat['Type'] == 'Controllable']))
    s4.metric("Uncontrollable", len(f_dsat[f_dsat['Type'] == 'Uncontrollable']))
    
    st.write("---")
    if not f_dsat.empty:
        # 3. Dynamic DSAT Table (Date, Advisor, Manager, Link, Type, Feedback, Action)
        col_widths = [1.5, 2, 2, 1.5, 1.5, 2.5, 1.2]
        h_cols = st.columns(col_widths)
        h_cols[0].write("**Date**"); h_cols[1].write("**Advisor**"); h_cols[2].write("**Manager**")
        h_cols[3].write("**Link**"); h_cols[4].write("**Type**"); h_cols[5].write("**Feedback**"); h_cols[6].write("**Action**")
        ...
        for _, row in f_dsat.iterrows():
            ...
            r_cols[0].write(str(row['Timestamp'])[:10])
            r_cols[1].write(row['Advisor Name'])
            r_cols[2].write(mgr)
            r_cols[3].markdown(f"[Chat URL]({row['DSAT chat link']})")
            r_cols[4].write(tp); r_cols[5].write(fb)
            if level in ["Manager", "Admin"]:
                if r_cols[6].button("Update", key=f"upd_{row['RecordKey']}"):
...
if level in ["Manager", "Admin"] and len(tabs) > 2:
    with tabs[2]:
        ...
        st.dataframe(sc.round(2), hide_index=True, use_container_width=True)

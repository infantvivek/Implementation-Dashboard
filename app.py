import streamlit as st
import pandas as pd
import numpy as np
import plotly.express as px
import plotly.graph_objects as go
import urllib.parse
import re
from streamlit.components.v1 import iframe

# --- 1. CONFIGURATION ---
TEAM_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSU-KDmKs9i1EIEuIuJTuKKxG4nFZoPluRqOonP2BxRbQuVJunS8WQ9uJA6ayUCdoq043uFMH6u3UcM/pub?gid=0&single=true&output=csv"
KPI_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSU-KDmKs9i1EIEuIuJTuKKxG4nFZoPluRqOonP2BxRbQuVJunS8WQ9uJA6ayUCdoq043uFMH6u3UcM/pub?gid=1918948844&single=true&output=csv"
DSAT_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vSU-KDmKs9i1EIEuIuJTuKKxG4nFZoPluRqOonP2BxRbQuVJunS8WQ9uJA6ayUCdoq043uFMH6u3UcM/pub?gid=367459010&single=true&output=csv"
LOGO_URL = "https://s3.amazonaws.com/cdn.freshdesk.com/data/helpdesk/attachments/production/48175265495/original/PTXBCP40UHx-8LCKsM1zqLX-pq8nndFHSw.png?1641235482"

# FORM CONFIG
FORM_ID = "YOUR_FORM_ID"
ENTRY_KEY, ENTRY_FEEDBACK, ENTRY_TYPE = "entry.1", "entry.2", "entry.3"

st.set_page_config(layout="wide", page_title="HighLevel Implementation Hub", page_icon="🚀")

# --- 2. SAAS UI CUSTOM CSS ---
st.markdown(f"""
    <style>
    /* Main Background and SaaS look */
    .main {{ background-color: #F8FAFC; }}
    .stApp {{ color: #0F172A; }}
    
    /* SaaS Metric Cards */
    [data-testid="stMetricValue"] {{ font-size: 28px !important; font-weight: 700 !important; color: #0052FF !important; }}
    .stMetric {{ 
        background-color: white !important; 
        padding: 24px !important; 
        border-radius: 16px !important; 
        box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06) !important;
        border: 1px solid #E2E8F0 !important;
    }}
    
    /* Sidebar Styling */
    [data-testid="stSidebar"] {{ background-color: #0F172A; }}
    [data-testid="stSidebar"] .stMarkdown {{ color: #CBD5E1; }}
    [data-testid="stSidebarNav"]::before {{
        content: ""; display: block; background-image: url('{LOGO_URL}');
        background-size: contain; background-repeat: no-repeat;
        width: 160px; height: 50px; margin: 30px auto; filter: brightness(0) invert(1);
    }}

    /* Tab Styling */
    .stTabs [aria-selected="true"] {{ 
        background-color: #0052FF !important; 
        color: white !important; 
        border-radius: 8px !important;
        font-weight: 600 !important;
    }}
    
    /* Narrative Box */
    div.stInfo {{ 
        background-color: #EFF6FF; 
        border: 1px solid #BFDBFE; 
        border-left: 5px solid #0052FF; 
        color: #1E40AF; 
        border-radius: 12px; 
        padding: 20px;
    }}
    </style>
""", unsafe_allow_html=True)

# --- 3. HELPER FUNCTIONS ---
def parse_time(time_str):
    if pd.isna(time_str) or not isinstance(time_str, str): return 0
    h, m = 0, 0
    try:
        parts = str(time_str).lower().split()
        for p in parts:
            if 'h' in p: h = int(re.sub(r'\D', '', p))
            elif 'm' in p: m = int(re.sub(r'\D', '', p))
        return (h * 60) + m
    except: return 0

@st.cache_data(ttl=60)
def load_and_standardize(url, sheet_type):
    try:
        df = pd.read_csv(url)
        df.columns = [re.sub(r'\s+', '_', str(c).strip().replace('\ufeff', '').replace('"', '')).lower() for c in df.columns]
        rmap = {
            "advisor_name": "advisor_name", "agent_name": "advisor_name",
            "advisor_email": "email", "email": "email",
            "manager": "manager_name", "access_level": "access_level",
            "ia": "ia_time", "advisor_call_time": "call_time",
            "q/a_calls": "qa_calls", "ob_calls": "ob_calls",
            "chat_dsat_url": "chat_url", "timestamp": "timestamp"
        }
        df = df.rename(columns=rmap)
        if 'email' in df.columns: df['email'] = df['email'].astype(str).str.strip().str.lower()
        if sheet_type == "KPI":
            for col in ['sent_rate_%', 'satisfied_survey_%', 'qa_calls', 'ob_calls', 'total_survey']:
                if col in df.columns:
                    df[col] = pd.to_numeric(df[col].astype(str).str.replace('%', ''), errors='coerce').fillna(0)
            df['ia_mins'] = df['ia_time'].apply(parse_time) if 'ia_time' in df.columns else 0
            df['call_mins'] = df['call_time'].apply(parse_time) if 'call_time' in df.columns else 0
            df['shift_score'] = np.where(df['ia_mins'] > 0, (df['call_mins']/df['ia_mins']*100), 0)
            if 'date_level_-_as' in df.columns:
                df['date_parsed'] = pd.to_datetime(df['date_level_-_as'], format="%b'%d'%y", errors='coerce')
        if sheet_type == "DSAT":
            df['timestamp_parsed'] = pd.to_datetime(df['timestamp'], errors='coerce')
        return df
    except: return pd.DataFrame()

def create_ghl_gauge(title, value, target=None):
    fig = go.Figure(go.Indicator(
        mode = "gauge+number", value = round(value, 2), domain = {'x': [0, 1], 'y': [0, 1]},
        title = {'text': title, 'font': {'size': 18, 'color': '#64748B', 'family': 'Inter'}},
        number = {'suffix': "%", 'font': {'color': '#0052FF', 'size': 38}, 'valueformat': '.2f'},
        gauge = {
            'axis': {'range': [0, 100], 'tickwidth': 1, 'tickcolor': "#CBD5E1"},
            'bar': {'color': "#0052FF"},
            'bgcolor': "white", 'borderwidth': 2, 'bordercolor': "#F1F5F9",
            'steps': [
                {'range': [0, 70], 'color': "#FEE2E2"},
                {'range': [70, 85], 'color': "#FEF3C7"},
                {'range': [85, 100], 'color': "#DCFCE7"}],
            'threshold': {'line': {'color': "#0F172A", 'width': 3}, 'thickness': 0.75, 'value': target} if target else None}
    ))
    fig.update_layout(height=240, margin=dict(l=35, r=35, t=50, b=20), paper_bgcolor='rgba(0,0,0,0)')
    return fig

@st.dialog("Update Record", width="large")
def open_form_dialog(row):
    params = {ENTRY_KEY: row.get('recordkey',''), ENTRY_FEEDBACK: row.get('feedback',''), ENTRY_TYPE: row.get('type','')}
    url = f"https://docs.google.com/forms/d/e/{FORM_ID}/viewform?usp=pp_url&{urllib.parse.urlencode(params)}"
    iframe(url, height=700, scrolling=True)
    if st.button("Close & Sync Data"): st.rerun()

# --- 4. AUTH & LOADING ---
if 'auth' not in st.session_state: st.session_state.auth = None
team_db = load_and_standardize(TEAM_URL, "TEAM")

if not st.session_state.auth:
    col_c1, col_c2, col_c3 = st.columns([1, 2, 1])
    with col_c2:
        st.image(LOGO_URL, width=200)
        with st.form("login_form"):
            st.markdown("### 👋 Welcome Back")
            u_email = st.text_input("Work Email").lower().strip()
            u_pass = st.text_input("Password", type="password")
            if st.form_submit_button("Sign In to Dashboard", use_container_width=True):
                match = team_db[(team_db['email'] == u_email) & (team_db['password'].astype(str) == str(u_pass))]
                if not match.empty:
                    st.session_state.auth = match.iloc[0].to_dict()
                    st.rerun()
                else: st.error("Invalid credentials.")
    st.stop()

# --- 5. DATA FETCHING ---
user = st.session_state.auth
kpi_raw = load_and_standardize(KPI_URL, "KPI")
dsat_raw = load_and_standardize(DSAT_URL, "DSAT")

# --- 6. NAVIGATION ---
st.sidebar.markdown("### 🛠 Navigation")
freq_mode = st.sidebar.radio("Timeframe Frequency", ["Daily", "Weekly", "Monthly", "Yearly"], horizontal=False)

# Frequency Context Logic
if freq_mode == "Daily":
    available = sorted(kpi_raw['date_parsed'].dropna().unique(), reverse=True)
    sel = st.sidebar.selectbox("Select Date", available, format_func=lambda x: x.strftime('%d-%m-%Y'))
    kpi_f, dsat_f = kpi_raw[kpi_raw['date_parsed'] == sel], dsat_raw[dsat_raw['timestamp_parsed'].dt.date == sel.date()]
elif freq_mode == "Weekly":
    kpi_raw['week_start'] = kpi_raw['date_parsed'].dt.to_period('W').apply(lambda r: r.start_time)
    available = sorted(kpi_raw['week_start'].dropna().unique(), reverse=True)
    sel = st.sidebar.selectbox("Select Week", available, format_func=lambda x: x.strftime('%d-%m-%Y'))
    kpi_f, dsat_f = kpi_raw[kpi_raw['week_start'] == sel], dsat_raw[(dsat_raw['timestamp_parsed'] >= sel) & (dsat_raw['timestamp_parsed'] < sel + pd.Timedelta(days=7))]
else:
    kpi_raw['month_label'] = kpi_raw['date_parsed'].dt.strftime('%B %Y')
    available = kpi_raw.sort_values('date_parsed', ascending=False)['month_label'].unique()
    sel = st.sidebar.selectbox("Select Period", available)
    kpi_f, dsat_f = kpi_raw[kpi_raw['month_label'] == sel], dsat_raw[dsat_raw['timestamp_parsed'].dt.strftime('%B %Y') == sel]

# Hierarchy
access = str(user.get('access_level', 'IC')).strip()
emails_to_scope = []
if access == "Admin":
    view_mode = st.sidebar.selectbox("View Scope", ["Entire Organisation", "Jarvis Sokolowich", "Sumit Ludhwani"])
    if view_mode == "Entire Organisation":
        emails_to_scope = team_db['email'].unique()
    else:
        mgr_list = team_db[team_db['manager_name'] == view_mode]['advisor_name'].unique()
        mgr_sel = st.sidebar.selectbox(f"Managers under {view_mode}", ["All Teams"] + list(mgr_list))
        if mgr_sel == "All Teams":
            emails_to_scope = team_db[team_db['manager_name'] == view_mode]['email'].unique()
        else:
            adv_list = team_db[team_db['manager_name'] == mgr_sel]['advisor_name'].unique()
            adv_sel = st.sidebar.selectbox(f"Advisor Drill-down", ["Full Team"] + list(adv_list))
            emails_to_scope = [team_db[team_db['advisor_name'] == adv_sel]['email'].values[0]] if adv_sel != "Full Team" else team_db[team_db['manager_name'] == mgr_sel]['email'].unique()
elif access == "Manager":
    view_mode = st.sidebar.selectbox("View Mode", ["Team Overview", "Individual Advisor View"])
    if view_mode == "Team Overview": emails_to_scope = team_db[team_db['manager_name'] == user['advisor_name']]['email'].unique()
    else: emails_to_scope = [team_db[team_db['advisor_name'] == st.sidebar.selectbox("Select Advisor", team_db[team_db['manager_name'] == user['advisor_name']]['advisor_name'].unique())]['email'].values[0]]
else: emails_to_scope = [user['email']]

f_kpi, f_dsat = kpi_f[kpi_f['email'].isin(emails_to_scope)], dsat_f[dsat_f['email'].isin(emails_to_scope)]

# --- 7. MAIN DASHBOARD ---
head_col1, head_col2 = st.columns([4, 1])
with head_col1:
    st.title("🚀 Implementation Performance Hub")
    st.markdown(f"**Welcome, {user['advisor_name']}** | Access: `{access}` | Context: `{sel}`")

tabs = st.tabs(["📊 Performance Overview", "🚫 DSAT Analysis", "🏆 Leaderboards"])

with tabs[0]:
    # a. Narrative
    avg_shift = f_kpi['shift_score'].mean() if not f_kpi.empty else 0
    st.info(f"**Summary Analysis:** The scoped group is maintaining an average Shift Score of **{avg_shift:.2f}%**. Performance is aligned with SaaS benchmarks for the period of **{sel}**.")
    
    # b. Metrics & Gauges
    st.divider()
    g1, g2, g3 = st.columns(3)
    avg_sent = (f_kpi['sent_rate_%'].mean() * 100) if not f_kpi.empty else 0
    avg_sat = (f_kpi['satisfied_survey_%'].mean() * 100) if not f_kpi.empty else 0
    g1.plotly_chart(create_ghl_gauge("Avg Survey Sent", avg_sent, 85), use_container_width=True)
    g2.plotly_chart(create_ghl_gauge("Avg Satisfied Survey", avg_sat, 90), use_container_width=True)
    g3.plotly_chart(create_ghl_gauge("Avg Shift Score", avg_shift, 85), use_container_width=True)
    
    m1, m2 = st.columns(2)
    with m1: st.metric("Total OB Calls", int(f_kpi['ob_calls'].sum()))
    with m2: st.metric("Total OH Calls (QA)", int(f_kpi['qa_calls'].sum()))

    # c. Trends
    st.markdown("### 📈 Time-Series Performance Trends")
    if not f_kpi.empty:
        trend = f_kpi.groupby('date_parsed').agg({'sent_rate_%':'mean', 'satisfied_survey_%':'mean', 'shift_score':'mean', 'ob_calls':'sum', 'qa_calls':'sum'}).reset_index()
        t1, t2 = st.columns(2)
        t1.plotly_chart(px.line(trend, x='date_parsed', y='sent_rate_%', title="Survey Sent Trend", color_discrete_sequence=['#0052FF']), use_container_width=True)
        t2.plotly_chart(px.line(trend, x='date_parsed', y='satisfied_survey_%', title="Satisfied Survey Trend", color_discrete_sequence=['#10B981']), use_container_width=True)
        
        t3, t4, t5 = st.columns(3)
        t3.plotly_chart(px.line(trend, x='date_parsed', y='shift_score', title="Shift Score Trend", color_discrete_sequence=['#F59E0B']), use_container_width=True)
        t4.plotly_chart(px.line(trend, x='date_parsed', y='ob_calls', title="OB Calls Trend", color_discrete_sequence=['#6366F1']), use_container_width=True)
        t5.plotly_chart(px.line(trend, x='date_parsed', y='qa_calls', title="OH Calls Trend", color_discrete_sequence=['#8B5CF6']), use_container_width=True)

with tabs[1]:
    st.markdown("### 🚫 DSAT Analysis & Auditing")
    pending = len(f_dsat[f_dsat['feedback'].isna() | (f_dsat['feedback'] == "")])
    s1, s2, s3, s4 = st.columns(4)
    s1.metric("Total DSATs", len(f_dsat))
    s2.metric("Pending Feedback", pending, delta=f"{pending} Unactioned", delta_color="inverse")
    s3.metric("Controllable", len(f_dsat[f_dsat['type'] == 'Controllable']))
    s4.metric("Uncontrollable", len(f_dsat[f_dsat['type'] == 'Uncontrollable']))

    st.divider()
    if not f_dsat.empty:
        # Table Formatter
        f_dsat_table = f_dsat.merge(team_db[['email', 'advisor_name', 'manager_name']], on='email', how='left')
        f_dsat_table['feedback'] = f_dsat_table['feedback'].fillna("-")
        f_dsat_table['type'] = f_dsat_table['type'].fillna("-")
        
        # Proper alignment using columns
        col_w = [1.2, 1.8, 1.5, 1, 1.2, 3] + ([1] if access != "IC" else [])
        headers = ["Date", "Advisor", "Manager", "Link", "Type", "Feedback"] + (["Action"] if access != "IC" else [])
        cols = st.columns(col_w)
        for i, h in enumerate(headers): cols[i].markdown(f"**{h}**")
        
        for idx, row in f_dsat_table.reset_index().iterrows():
            r = st.columns(col_w)
            r[0].write(str(row['timestamp'])[:10])
            r[1].write(row['advisor_name_x'])
            r[2].write(row['manager_name_x'])
            r[3].markdown(f"[Chat]({row['chat_url']})")
            r[4].write(row['type'])
            r[5].write(row['feedback'])
            if access != "IC" and r[6].button("Update", key=f"upd_{idx}", use_container_width=True):
                open_form_dialog(row)

with tabs[2]:
    st.markdown("### 🏆 Performance Leaderboards")
    if not kpi_f.empty:
        ldb = kpi_f.groupby('advisor_name').agg({'sent_rate_%':'mean', 'satisfied_survey_%':'mean', 'qa_calls':'sum', 'ob_calls':'sum'}).reset_index()
        ldb['sent_rate_%'] *= 100
        ldb['satisfied_survey_%'] *= 100
        
        # Success Champions
        st.info("**Success Champions:** Advisors with Sent Rate ≥ 85% and Satisfied % ≥ 90%")
        champs = ldb[(ldb['sent_rate_%'] >= 85) & (ldb['satisfied_survey_%'] >= 90)].sort_values('satisfied_survey_%', ascending=False)
        st.dataframe(champs[['advisor_name', 'satisfied_survey_%', 'sent_rate_%']].style.format({'satisfied_survey_%': '{:.2f}%', 'sent_rate_%': '{:.2f}%'}), use_container_width=True, hide_index=True)
        
        st.divider()
        c_l1, c_l2 = st.columns(2)
        with c_l1:
            st.markdown("#### Total QA Calls")
            st.dataframe(ldb.sort_values('qa_calls', ascending=False)[['advisor_name', 'qa_calls']], hide_index=True, use_container_width=True)
            st.markdown("#### Avg Satisfied Survey")
            st.dataframe(ldb.sort_values('satisfied_survey_%', ascending=False)[['advisor_name', 'satisfied_survey_%']].style.format({'satisfied_survey_%': '{:.2f}%'}), hide_index=True, use_container_width=True)
        with c_l2:
            st.markdown("#### Total OB Calls")
            st.dataframe(ldb.sort_values('ob_calls', ascending=False)[['advisor_name', 'ob_calls']], hide_index=True, use_container_width=True)
            st.markdown("#### Avg Survey Sent")
            st.dataframe(ldb.sort_values('sent_rate_%', ascending=False)[['advisor_name', 'sent_rate_%']].style.format({'sent_rate_%': '{:.2f}%'}), hide_index=True, use_container_width=True)

st.sidebar.divider()
if st.sidebar.button("Logout", use_container_width=True):
    st.session_state.auth = None
    st.rerun()

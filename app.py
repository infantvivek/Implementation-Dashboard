...
st.markdown("""
    ...
    [data-testid="stSidebarNav"]::before {
        ...
        width: 180px; height: 60px; margin-left: 20px; margin-top: 20px; filter: brightness(0) invert(1);
    }
...
def load_and_standardize(url, sheet_type):
    try:
        ...
        if sheet_type == "KPI":
            # Fix: Ensure percentages are read correctly and NOT multiplied by 100 here
...
def create_ghl_gauge(title, value, target=None):
    fig = go.Figure(go.Indicator(
        ...
        number = {'suffix': "%", 'font': {'color': '#0052FF', 'size': 40}},
...
if freq_mode == "Daily":
    ...
    dsat_f = dsat_raw[dsat_raw['timestamp_parsed'].dt.strftime('%B %Y') == sel] if freq_mode == "Monthly" else dsat_raw[dsat_raw['timestamp_parsed'].dt.year == sel]
...
if access == "Admin":
    ...
    if view_mode == "Entire Organisation":
        ...
        mgrs = team_db[team_db['manager_name'] == view_mode]['advisor_name'].unique()
        mgr_sel = st.sidebar.selectbox(f"Managers under {view_mode}", ["All Teams"] + list(mgrs))
        if mgr_sel == "All Teams":
            emails = team_db[team_db['manager_name'] == view_mode]['email'].unique()
        else:
...
with tab1:
    if not f_kpi.empty:
        ...
        # Fix: Multiply by 100 only if values are recorded as decimals in sheet
        raw_sent = f_kpi[f_kpi['total_survey'] > 0]['sent_rate_%'].mean() if not f_kpi.empty else 0
        avg_sent = raw_sent * 100 if raw_sent <= 1 else raw_sent
        
        raw_sat = f_kpi[f_kpi['total_survey'] > 0]['satisfied_survey_%'].mean() if not f_kpi.empty else 0
...
with tab2:
    ...
    st.dataframe(f_dsat[['timestamp', 'chat_url', 'type', 'feedback']].fillna("-"), hide_index=True, use_container_width=True)

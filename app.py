...
st.markdown("""
    <style>
    /* SIGNATURE LOGO BIGGER & INVERTED FOR DARK MODE */
    .ghl-logo { display: block; margin-left: auto; margin-right: auto; width: 300px; padding: 20px; filter: invert(1); }
...
def create_ghl_gauge(title, value, target, is_percent=True, color_steps=False):
    # Standardized 0-100 range for all gauges
    steps = []
    if color_steps:
        # Standard Red (0-70), Yellow (70-85), Green (85-100) indicators
    ...
    # standardized heights and margins
...
st.caption(f"Logged: {user['Advisor Name']} | Access: {level} | Period: {sel}")
...
with tabs[0]:
    ...
    # Hero Gauge Row (Shift Score, Sent Rate, Sat Survey)
    c_g1, c_g2, c_g3 = st.columns(3)
    # HERO GAUGES with Standardization 0-100 & Red/Orange/Green Steps
    c_g1.plotly_chart(create_ghl_gauge("Shift Score", avg_score, 85, is_percent=True, color_steps=True), use_container_width=True)
    c_g2.plotly_chart(create_ghl_gauge("Survey Sent %", avg_sent, 85, is_percent=True, color_steps=True), use_container_width=True)
    c_g3.plotly_chart(create_ghl_gauge("Satisfied Survey %", avg_sat, 90, is_percent=True, color_steps=True), use_container_width=True)

    # Secondary Metric Cards (OB/QA)
    total_ob = f_kpi['OB Calls'].sum() if not f_kpi.empty else 0
    total_qa = f_kpi['Q/A Calls'].sum() if not f_kpi.empty else 0
    col_v1, col_v2, *others = st.columns(4)
    col_v1.metric("Total OB Calls", int(total_ob)); col_v2.metric("Total QA Calls", int(total_qa))

    # Trends
    trend_data = f_kpi.groupby('Date_Parsed').mean(numeric_only=True).reset_index()
    c_t1, c_t2 = st.columns(2)
    with c_t1: st.plotly_chart(px.line(trend_data, x='Date_Parsed', y='Shift_Score', title="Shift Score Trend", markers=True, color_discrete_sequence=['#0052FF']), use_container_width=True)
    with c_t2: st.plotly_chart(px.line(trend_data, x='Date_Parsed', y='Satisfied Survey %', title="Satisfied Survey Trend", markers=True, color_discrete_sequence=['#22c55e']), use_container_width=True)
...
with tabs[2]:
    ...
    ldb = f_kpi[f_kpi['Total Survey'] > 0].groupby('Advisor Name').agg({'Sent Rate %':'mean', 'Satisfied Survey %':'mean'}).reset_index().round(2)

...
# PRE-FILLED FORM CONFIG (Update with your actual Form ID and entry IDs)
...
def generate_form_url(row):
    # Generates pre-filled URL for the Google Form
...
def open_form_dialog(url):
    st.write("Submit the form below to update the Google Sheet record.")
    # Embedding the Google Form via Iframe
    st.components.v1.iframe(url, height=700, scrolling=True)
    if st.button("Done / Close"):
        st.rerun()
...
def load_data(url, sheet_type=None):
    try:
        ...
        mappings = {
            ...
            "DSAT": {"Advisor Email": "Email", "Chat DSAT URL": "DSAT chat link", "Manager": "Manager Name", "Advisor Name": "Advisor Name"}
        }
...
if level == "Admin":
    ...
    if scope == "Global": f_kpi, f_dsat = f_kpi_t, f_dsat_t
        ...
        ems = team_db[team_db['Manager Name'] == m_name]['Email'].unique()
...
with tabs[0]:
    ...
    ca, cb = st.columns(2); chart_d = f_kpi.groupby('Date_Parsed').mean(numeric_only=True).reset_index() if not f_kpi.empty else pd.DataFrame()
    with ca:
...
with tabs[1]:
    ...
    if not f_dsat.empty:
        # Table Header
        h_cols = st.columns([2, 1.5, 3.5, 1.5, 1.5])
        h_cols[0].write("**Date**"); h_cols[1].write("**Link**"); h_cols[2].write("**Feedback**"); h_cols[3].write("**Type**")
        if level in ["Manager", "Admin"]: h_cols[4].write("**Action**")
        ...
        for _, row in f_dsat.iterrows():
            ...
            r_cols = st.columns([2, 1.5, 3.5, 1.5, 1.5])
            r_cols[0].write(str(row['Timestamp'])[:10])
            r_cols[1].markdown(f"[Chat URL]({row['DSAT chat link']})")
            r_cols[2].write(fb); r_cols[3].write(tp)
            if level in ["Manager", "Admin"]:
                # Open Google Form in Dialog (Iframe popup)
                if r_cols[4].button("Update", key=f"upd_{row['RecordKey']}"):
...
if level in ["Manager", "Admin"] and len(tabs) > 2:
    with tabs[2]:
        ...
        with l1:
            st.write("**Success Champions**"); sc = ldb[(ldb['Sent Rate %'] >= 85) & (ldb['Satisfied Survey %'] > 90)].sort_values(['Satisfied Survey %','Sent Rate %'], ascending=False)
